...
class CardDetailsEditView: UIView, AddPaymentMethodView, CardScanningViewDelegate {
    ...
    var shouldSavePaymentMethod: Bool {
        return saveThisCardCheckbox.isEnabled && saveThisCardCheckbox.isOn
    }
    ...
    lazy var saveThisCardCheckbox: UISwitch = {
        let saveThisCardCheckbox = UISwitch()
        saveThisCardCheckbox.addTarget(self, action: #selector(didSelectSaveThisCard), for: .valueChanged)
        saveThisCardCheckbox.setOn(true, animated: false)
  ...
  @objc func scanButtonTapped(_ button: UIButton) {
    ...
    init(canSaveCard: Bool, billingAddressCollection: PaymentSheet.BillingAddressCollectionLevel, merchantDisplayName: String, delegate: AddPaymentMethodViewDelegate) {
        ...
        contentView.setCustomSpacing(16, after: formView)
