...
class ConfirmButton: UIView {
    ...
    func update(state: Status? = nil,
        ...
        let shouldSwapButtons = style == .applePay && applePayButton.isHidden || style != .applePay && buyButton.isHidden
        ...
        UIView.animate(withDuration: PaymentSheetUI.quickAnimationDuration) {
            self.layer.shadowOpacity = state == .disabled ? 0 : Self.shadowOpacity
        }

        // Update the buy button; it has its own presentation logic
        self.buyButton.update(status: self.state, callToAction: self.callToAction)
    }
    ...
    fileprivate class BuyButton: UIControl {
        ...
        lazy var spinnerCenteredToLockConstraint: NSLayoutConstraint = {
            return spinner.centerXAnchor.constraint(equalTo: lockIcon.centerXAnchor)
        }()
