...
extension PaymentSheet {
    ...
    public class FlowController {
        ...
        public func presentPaymentOptions(from presentingViewController: UIViewController, completion: (() -> ())? = nil) {
            assert(presentingViewController.presentedViewController == nil, "The view controller is already presenting something!")
        ...
        public func confirmPayment(from presentingViewController: UIViewController, completion: @escaping (PaymentResult) -> ()) {
            ...
            let paymentHandlerCompletion: STPPaymentHandlerActionPaymentIntentCompletionBlock = { [weak self] (status, paymentIntent, error) in
                ...
                switch status {
                case .canceled:
                    completion(.canceled(lastError: nil, paymentIntent: paymentIntent))
                case .failed:
                    let error: Error = error ?? PaymentSheetError.unknown(debugDescription: "STPPaymentHandler failed without an error")
                    completion(.failed(error: error, paymentIntent: paymentIntent))
                case .succeeded:
                    completion(.completed(paymentIntent: self.paymentIntent))
                }
            }
            ...
            case .applePay:
                guard let applePayConfiguration = configuration.applePay,
                    ...
                    assertionFailure("Apple Pay was selected but is not supported by the device")
                    let error = PaymentSheetError.unknown(debugDescription: "Apple Pay was selected but is not supported by the device")
                    completion(.failed(error: error, paymentIntent: paymentIntent))
                    return
                }
